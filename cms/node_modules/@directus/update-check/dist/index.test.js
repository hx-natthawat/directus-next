...
beforeEach(() => {
    ...
    mockResponse = { ok: true, json: vi.fn().mockResolvedValue(mockResponseJson) };
...
test('Throws error if response is not ok', async () => {
    ...
    try {
        ...
        expect(err.message).toBe(`Couldn't find latest version for package "${sample.name}": ${mockResponse.status} ${mockResponse.statusText}`);
    }
});
